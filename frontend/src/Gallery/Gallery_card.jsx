...
const GalleryCard = ({ items }) => {
  ...
  return (
    <div className="w-full p-6 mx-auto">
      <div onClick={() => document.getElementById(items.id).showModal()} className="shadow-md hover:shadow-xl rounded bg-white overflow-hidden cursor-pointer">
        ...
        <div className="p-3">
          <div className="flex flex-col cursor-pointer space-y-3 md:flex justify-between">
            <div className="flex flex-row place-content-between">
              ...
              <span className="text-sm clamp text-gray-500">{items.date}</span>
            </div>
            <div>
              <span className="text-sm line-clamp-2 md:line-clamp-3 text-gray-500">{items.description}</span>
            </div>
          </div>
        </div>
      </div>
